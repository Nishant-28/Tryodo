...
const MobileOrderCard: React.FC<OrderCardProps> = ({ order, onTrackOrder }) => {
  ...
  const getProgressSteps = () => {
    ...
    const steps = [
      ...
      },
      {
        ...
        timestamp: order.out_for_delivery_date || (currentStatus.toLowerCase() === 'out_for_delivery' ? order.updated_at : null),
        icon: Truck,
  ...
  return (
    <Card className="w-full mb-4 shadow-sm border border-gray-200 bg-white rounded-lg overflow-hidden">
      <CardHeader className="pb-3 px-4 pt-4">
        {/* Order Header */}
        <div className="flex justify-between items-start gap-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-gray-900 text-base truncate">{order.order_number}</h3>
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <Badge className={cn("text-xs font-medium px-2 py-1", getStatusColor(currentStatus))}>
                {currentStatus.replace('_', ' ').toUpperCase()}
              </Badge>
              <Badge className={cn("text-xs font-medium px-2 py-1", getPaymentStatusColor(order.payment_status))}>
        ...
        <div className="flex items-center justify-between mt-3 text-sm text-gray-600">
          ...
          {onTrackOrder && (
            <Button 
              ...
              className="text-blue-600 border-blue-200 hover:bg-blue-50 h-8 px-3"
            >
              Track Order
            </Button>
          )}
      ...
      <CardContent className="pt-0 px-4 pb-4">
              ...
              <div className="flex justify-between items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    by {order.order_items[0].vendor?.business_name || 'Vendor Info Missing'}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-600">Qty: {order.order_items[0].quantity}</p>
                  <p className="text-xs font-medium">₹{order.order_items[0].unit_price} each</p>
                  <p className="text-sm font-bold">₹{order.order_items[0].line_total} total</p>
                </div>
              </div>
              <div className={cn(
                "inline-block px-2 py-1 rounded text-xs font-medium mt-2",
                getStatusColor(order.order_items[0].item_status || currentStatus)
              )}>
                {(order.order_items[0].item_status || currentStatus).replace('_', ' ').toUpperCase()}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <h4 className="font-medium text-gray-900 text-sm">Delivery Progress</h4>
        ...
        <Button
          ...
          className="w-full justify-center text-gray-600 h-8 text-sm"
        >
          {isExpanded ? (
            <>
              Show Less
        ...
        {isExpanded && (
          <div className="mt-4 space-y-4">
            ...
            {order.order_items && order.order_items.length > 1 && (
              <div>
                <h4 className="font-medium text-gray-900 mb-2 text-sm">All Order Items</h4>
                <div className="space-y-2">
                    ...
                    <div key={item.id || index} className="bg-gray-50 rounded-lg p-3">
                      <div className="flex justify-between items-start gap-3">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs font-medium">₹{item.unit_price} each</p>
            ...
            {/* Order Details */}
            <div>
              <h4 className="font-medium text-gray-900 mb-3 text-sm">Order Details</h4>
              <div className="grid grid-cols-1 gap-3 text-sm">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 text-xs">Order Number:</span>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-gray-600 text-xs">Estimated Delivery:</span>
                  <span className="font-medium text-xs">{formatEstimatedDelivery(order.estimated_delivery_date)}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="bg-gray-50 p-3 rounded-lg text-sm">
                <p className="font-semibold text-base text-gray-900 mb-2">Delivery Address</p>
                {customerAddress ? (
            ...
            {order.delivery_otp && currentStatus !== 'delivered' && (
              <div>
                <h4 className="font-medium text-gray-900 mb-2 text-sm">Delivery OTP</h4>
                <div className="bg-blue-50 p-3 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-2xl font-bold text-blue-900">{order.delivery_otp}</p>
                      <p className="text-xs text-blue-700">Share this OTP with delivery partner</p>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => copyOTP(order.delivery_otp!)}
                      className="text-blue-600 border-blue-200 h-8 w-8 p-0"
                    >
            ...
            {order.delivery_partner_name && (
              <div>
                ...
                <div className="bg-gray-50 p-3 rounded-lg text-sm">
                  <p className="font-medium text-sm">{order.delivery_partner_name}</p>
                  {order.delivery_partner_phone && (
                    <p className="text-gray-600 flex items-center gap-1 mt-1 text-xs">
