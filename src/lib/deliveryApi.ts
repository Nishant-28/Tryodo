...
export class DeliveryAPI {
  ...
  static async markDelivered(
    ...
    try {
      ...
      const { data: currentOrder, error: orderCheckError } = await supabase
        .from('orders')
        .select('order_status, actual_delivery_date')
        .eq('id', orderId)
        .single();

      if (orderCheckError) throw orderCheckError;

      // If order is already delivered, return success
      if (currentOrder?.order_status === 'delivered') {
        return { 
          success: true, 
          data: { message: 'Order already marked as delivered.' } 
        };
      }

      // Check delivery partner assignment
      const { data: deliveryAssignment, error: assignmentError } = await supabase
        .from('delivery_partner_orders')
      ...
      // If delivery assignment is already marked as delivered, only update main order
      if (deliveryAssignment?.status === 'delivered') {
        console.log('Delivery assignment already marked as delivered, updating main order only');
        
        const deliveredAt = deliveryAssignment.delivered_at || new Date().toISOString();

        // Update main order status only
        ...
        return { success: true, data: { message: 'Order status synchronized successfully.' } };
      }

      // Start normal delivery marking process
      ...
      // 2. Update order_items table: set item_status to 'delivered' (only if not already delivered)
      const { error: orderItemsError } = await supabase
        ...
        .eq('order_id', orderId)
        .neq('item_status', 'delivered'); // Only update items that are not already delivered
      ...
      const { error: ordersError } = await supabase
        ...
        .eq('id', orderId)
        .neq('order_status', 'delivered'); // Only update if not already delivered

      if (ordersError) throw ordersError;

      // 4. Update order_deliveries record (created earlier) to mark as delivered
      const { error: deliveryRecError } = await supabase
   ...
   * @param deliveryPartnerId - The ID of the delivery partner cancelling the order
   * @param reason - The reason for cancellation
   * @param additionalDetails - Additional details about the cancellation
   */
  static async cancelOrder(
    ...
    try {
      console.log('🚫 Cancelling order:', { orderId, deliveryPartnerId, reason });

      // Validate inputs
      if (!orderId || !deliveryPartnerId || !reason) {
        return {
          success: false,
          error: 'Missing required parameters: orderId, deliveryPartnerId, and reason are required'
        };
      }

      // Check if order can be cancelled
      const canCancelResult = await this.canCancelOrder(orderId, deliveryPartnerId);
      if (!canCancelResult.success || !canCancelResult.data?.canCancel) {
        return {
          success: false,
          error: canCancelResult.data?.reason || canCancelResult.error || 'Order cannot be cancelled'
        };
      }

      // Start transaction by creating cancellation record
      ...
      if (dpOrderUpdateError) {
        console.error('❌ Error updating delivery partner order:', dpOrderUpdateError);
        throw dpOrderUpdateError;
      }
      ...
      this.triggerCancellationNotifications(orderId, deliveryPartnerId, reason, additionalDetails)
...
export class DeliveryAPI {
  ...
  /**
   * Get cancelled orders for a vendor
   * @param vendorId - The ID of the vendor
   * @param filters - Optional filters for the query
   */
  static async getVendorCancelledOrders(
    vendorId: string,
    filters: CancellationFilters = {}
  ): Promise<ApiResponse<PaginatedResponse<CancelledOrderDetails>>> {
    try {
      console.log('📋 Fetching cancelled orders for vendor:', vendorId, filters);
      ...
      // Build the query - get cancelled orders that have items from this vendor
      let query = supabase
        .from('orders')
        .select(`
          id,
      ...
      // Apply filters
      if (start_date) {
        query = query.gte('cancelled_date', start_date);
      }
      if (end_date) {
        query = query.lte('cancelled_date', end_date);
      }
      if (cancellation_reason) {
        query = query.ilike('cancellation_reason', `%${cancellation_reason}%`);
      }
      ...
      // Order by cancellation date (most recent first)
      query = query.order('cancelled_date', { ascending: false });

      const { data, error, count } = await query;

      if (error) {
        console.error('❌ Database error fetching vendor cancelled orders:', error);
        throw error;
      }

      console.log('📊 Query result:', { dataLength: data?.length || 0, count });

      // Handle case where no data is returned
      if (!data || data.length === 0) {
        console.log('ℹ️ No cancelled orders found for vendor:', vendorId);
        return {
          success: true,
          data: {
            data: [],
      ...
      // Transform the data
      const cancelledOrders: CancelledOrderDetails[] = (data || []).map(item => {
        // Filter order items to only include items from this vendor
        const vendorItems = item.order_items.filter((orderItem: any) => orderItem.vendor_id === vendorId);
        ...
        return {
          ...
          vendor_items: vendorItems.map((orderItem: any) => ({
            product_name: orderItem.product_name,
            quantity: orderItem.quantity,
            line_total: orderItem.line_total
          })),
          cancellation: {
            id: item.id, // Use order id since there's no separate cancellation record
            order_id: item.id,
            delivery_partner_id: null, // Customer cancellations don't have delivery partner
            cancellation_reason: item.cancellation_reason || 'Order cancelled by customer',
            additional_details: null,
            cancelled_at: item.cancelled_date,
            created_at: item.created_at,
            updated_at: item.updated_at
          },
          delivery_partner_name: 'Customer Cancellation',
          cancelled_at: item.cancelled_date,
          vendor_names: [vendorId] // This vendor only
        };
      });
      ...
      console.error('💥 Error fetching vendor cancelled orders:', error);
      return {
        success: false,
        error: error.message || 'Failed to fetch cancelled orders'
      };
    }
  }
   ...
   */
  static async getCancellationAnalytics(
    filters: CancellationFilters = {}
  ): Promise<ApiResponse<any>> {
    try {
      console.log('📊 Fetching cancellation analytics:', filters);
      ...
      const startDateFilter = startDate || start_date || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
      const endDateFilter = endDate || end_date || new Date().toISOString().split('T')[0];

      // Get cancellation data from the 'orders' table where order_status is 'cancelled'
      let cancellationQuery = supabase
        ...
        .select(`
          ...
          order_items(vendor_id, vendor_product_id, market_vendor_product_id, product_type, quantity)
        `)
        .eq('order_status', 'cancelled')
        .gte('cancelled_date', startDateFilter)
        .lte('cancelled_date', endDateFilter + 'T23:59:59.999Z');

      // Apply vendor and delivery partner filters by joining with order_items and delivery_partner_orders
      if (vendor_id) {
        cancellationQuery = cancellationQuery.eq('order_items.vendor_id', vendor_id);
      }
      if (delivery_partner_id) {
        cancellationQuery = cancellationQuery.eq('delivery_partner_orders.delivery_partner_id', delivery_partner_id);
      }

      const { data: cancellationData, error: cancellationError } = await cancellationQuery;
      if (cancellationError) throw cancellationError;

      // Get total orders for rate calculation (this query is correct as it queries the orders table)
      let totalOrdersQuery = supabase
        .from('orders')
        .select('id', { count: 'exact', head: true })
        .gte('created_at', startDateFilter)
        .lte('created_at', endDateFilter + 'T23:59:59.999Z');

      if (vendor_id) {
        // For total orders count, we need to consider orders that have items from the vendor
        totalOrdersQuery = totalOrdersQuery
          .eq('order_items.vendor_id', vendor_id);
      }

      const { count: totalOrders } = await totalOrdersQuery;

      // Calculate basic metrics
      const totalCancellations = cancellationData?.length || 0;
      const cancellationRate = totalOrders ? (totalCancellations / totalOrders) * 100 : 0;
      
      console.log('📊 Analytics data:', {
        totalCancellations,
        totalOrders,
        cancellationRate,
        cancellationDataLength: cancellationData?.length
      });

      // Calculate total value lost and average cancelled order value
      const totalValueLost = cancellationData?.reduce((sum, item) => sum + (item.total_amount || 0), 0) || 0;
      const averageCancelledOrderValue = totalCancellations > 0 ? totalValueLost / totalCancellations : 0;

      // Group by reason for top reasons
      const reasonGroups = new Map<string, number>();
      cancellationData?.forEach(item => {
        const reason = item.cancellation_reason || 'Unknown';
        const current = reasonGroups.get(reason) || 0;
        reasonGroups.set(reason, current + 1);
      });

      const topReasons = Array.from(reasonGroups.entries())
        .sort(([, countA], [, countB]) => countB - countA)
        .map(([reason, count]) => ({ reason, count, percentage: (count / totalCancellations) * 100 })); // Calculate percentage here
      ...
      cancellationData?.forEach(item => {
        const date = new Date(item.cancelled_date).toISOString().split('T')[0]; // Use cancelled_date
        const current = dailyTrends.get(date) || {
          date,
          cancellations: 0,
          valueLost: 0
        };
        current.cancellations++;
        current.valueLost += item.total_amount || 0; // Use total_amount
      ...
      // Prepare cancellation details with more context
      const detailedCancellations = cancellationData?.map(item => ({
        id: item.id,
        order_id: item.id, // order_id is the id of the order
        order_number: item.order_number,
        total_amount: item.total_amount,
        cancellation_reason: item.cancellation_reason,
        additional_details: item.additional_details, // Assuming this is available in orders table
        cancelled_by: item.cancelled_by || 'Customer', // Default to Customer if not explicitly set
        cancelled_at: item.cancelled_date, // Use cancelled_date
        customer_name: item.customers?.[0]?.profiles?.[0]?.full_name || 'N/A', // Access customer name correctly
        created_at: item.created_at, // Original order creation date
      }));
      ...
      return {
        ...
        data: {
          ...
          cancellationRate,
          totalValueLost,
          averageCancelledOrderValue,
          topReasons,
          trends,
          detailedCancellations,
          // Add dummy data for peakCancellationHour and cancellationTrend for now, will address later if needed
          peakCancellationHour: 12, // Dummy value
          cancellationTrend: 0, // Dummy value
        }
      };
    } catch (error: any) {
      ...
      return {
        success: false,
        error: error.message || 'Failed to fetch cancellation analytics'
      };
    }
  }
  ...
  /**
   * Get cancelled orders for a delivery partner
   * @param deliveryPartnerId - The ID of the delivery partner
   * @param filters - Optional filters for the query
   */
  static async getDeliveryPartnerCancelledOrders(
    deliveryPartnerId: string,
    filters: CancellationFilters = {}
  ): Promise<ApiResponse<PaginatedResponse<CancelledOrderDetails>>> {
    try {
      console.log('📋 Fetching cancelled orders for delivery partner:', deliveryPartnerId, filters);
      ...
      let query = supabase
        ...
        .select(`
          *,
          orders!order_cancellations_order_id_fkey!inner(
            id,
            order_number,
            total_amount,
            customers(
      ...
      if (end_date) {
        query = query.lte('cancelled_at', end_date);
      }
      if (cancellation_reason) {
        query = query.eq('cancellation_reason', cancellation_reason);
      }
      ...
      const cancelledOrders: CancelledOrderDetails[] = (data || []).map(item => ({
        ...
        customer_name: item.orders.customers?.[0]?.profiles?.[0]?.full_name || 'Unknown',
        total_amount: item.orders.total_amount,
        cancellation: {
          id: item.id,
          order_id: item.order_id,
          delivery_partner_id: item.delivery_partner_id,
          cancellation_reason: item.cancellation_reason,
        ...
        delivery_partner_name: item.delivery_partners?.profiles?.[0]?.full_name || 'Unknown',
        cancelled_at: item.cancelled_at,
        vendor_names: [] // Would need additional query to get vendor names
      }));
      ...
      console.error('💥 Error fetching delivery partner cancelled orders:', error);
      return {
        success: false,
        error: error.message || 'Failed to fetch cancelled orders'
      };
   ...
   * @param deliveryPartnerId - The ID of the delivery partner
   */
  static async canCancelOrder(
    orderId: string,
    deliveryPartnerId: string
  ): Promise<ApiResponse<{ canCancel: boolean; reason?: string }>> {
    try {
      // Check order status and delivery partner assignment
      const { data: orderData, error: orderError } = await supabase
        .from('orders')
        .select(`
          id,
          order_status,
          cancellation_id,
          delivery_partner_orders!inner(
            delivery_partner_id,
            status
          )
        `)
        .eq('id', orderId)
        .eq('delivery_partner_orders.delivery_partner_id', deliveryPartnerId)
        .single();

      if (orderError) {
        return {
          success: false,
          error: 'Order not found or not assigned to this delivery partner'
        };
      }

      // Check if order is already cancelled
      if (orderData.cancellation_id) {
        return {
          success: true,
          data: {
            canCancel: false,
      ...
      const validStatuses = ['out_for_delivery', 'picked_up'];
      if (!validStatuses.includes(orderData.order_status)) {
        return {
          success: true,
          data: {
            canCancel: false,
            reason: `Order status '${orderData.order_status}' does not allow cancellation`
          }
        };
      }
  ...
  /**
   * Trigger notifications for order cancellation (internal method)
   */
  static async triggerCancellationNotifications(
    orderId: string,
    deliveryPartnerId: string,
    reason: CancellationReason,
    additionalDetails?: string
    ...
    try {
      console.log('📧 Triggering cancellation notifications for order:', orderId);
      ...
      const { data: deliveryPartnerDetails, error: dpError } = await supabase
        .from('delivery_partners')
        .select(`
          id,
          profiles(full_name)
      ...
      if (dpError) {
        console.error('❌ Error fetching delivery partner details:', dpError);
        return;
      }

      const deliveryPartnerName = deliveryPartnerDetails?.profiles?.full_name || 'Delivery Partner';

      // TODO: Implement actual notification sending
      // This would integrate with your notification system (email, SMS, push notifications)
      
      console.log('📧 Notifications would be sent to:');
      console.log('- Customer:', orderDetails.customers?.profiles?.full_name);
      console.log('- Vendors:', orderDetails.order_items?.map(item => item.vendors?.business_name));
      console.log('- Reason:', reason);
      console.log('- Cancelled by:', deliveryPartnerName);
      ...
      console.error('💥 Error triggering cancellation notifications:', error);
...
// All APIs are exported individually where they are defined above
